...
for booking in data_dict['bookings']:
    ...
    b = Booking(booking['id'], count, booking['price'],
                booking['maximumDuration'], booking['passengers'], jobs, time_table_dict[jobs[0].station, jobs[1].station])
...
for shift in data_dict['shifts']:
    ...
    s = Shift(shift['id'], shift['capacity'],
              shift['maximumTurnover'], jobs, time_table_dict)
...
tw_driver_dict = {}
for driver in shifts:
    tw_driver_dict[driver.long_id] = (driver.depots[0].time_date, driver.depots[1].time_date)
